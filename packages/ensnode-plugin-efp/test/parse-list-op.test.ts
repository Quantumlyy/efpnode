...
describe("parseListOp", () => {
  it("decodes version, opcode, and data payload", () => {
    ...
    const op = ("0x0101" + "01" + "01" + A20.slice(2)) as `0x${string}`;
    expect(parseListOp(op)).toEqual({
      version: 1,
      opcode: 1,
      data: ("0x" + "01" + "01" + A20.slice(2)) as `0x${string}`,
    });
  });
...
describe("extractTargetAddress", () => {
  it("extracts the 20-byte address from an ADD_RECORD op", () => {
    // version(01) + opcode(01) + recordVersion(01) + recordType(01) + address
    const op = ("0x01010101" + "cc".repeat(20)) as `0x${string}`;
    expect(extractTargetAddress(op)).toBe("0x" + "cc".repeat(20));
  });

  it("lowercases the extracted address", () => {
    const op = ("0x01010101" + "AA".repeat(20)) as `0x${string}`;
    expect(extractTargetAddress(op)).toBe("0x" + "aa".repeat(20));
  });

  it("returns empty string for too-short payloads", () => {
    expect(extractTargetAddress("0x01010101")).toBe("");
    expect(extractTargetAddress(null)).toBe("");
  });
});
